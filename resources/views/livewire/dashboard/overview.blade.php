<div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-4">
            ...
            @if ($activeGoal)
                <div class="flex items-center gap-2 rounded-full border border-kb-blue-electric/20 bg-kb-blue-electric/5 px-3 py-1.5">
        ...
        <flux:button variant="primary" :href="route('snapshot.create')" class="btn-gradient-primary rounded-xl">Update Data Penjualan</flux:button>
    </div>
    ...
    @if ($passport)
        ...
        @if (count($timelineSteps) > 0)
            <flux:card class="p-5 rounded-2xl border-kb-border bg-kb-surface-1">
                <div class="flex items-center justify-between mb-4">
                    <flux:heading level="2" class="font-display text-lg font-semibold text-kb-text-primary">Timeline Plan Action</flux:heading>
                    <a href="{{ route('action-plan.index') }}" class="text-sm font-medium text-kb-blue-electric hover:underline">
                        Lihat Semua
                    </a>
                </div>
                    ...
                    <div class="flex items-start min-w-max">
                        @foreach ($timelineSteps as $index => $step)
                            <div class="flex items-start">
                                {{-- Connector line --}}
                                @if ($index > 0)
                                    <div class="w-10 h-px bg-kb-border mt-4 shrink-0 {{ $step['status'] === 'pending' ? 'opacity-50' : '' }}"></div>
                                @endif
                                ...
                                <div class="flex flex-col items-center w-36 shrink-0 text-center">
                                    <div @class([
                                        ...
                                        'bg-kb-blue-electric border-kb-blue-electric text-white' => $step['status'] === 'done',
                                        'bg-white border-kb-blue-electric text-kb-blue-electric timeline-node-running' => $step['status'] === 'today',
                                        'bg-white border-kb-border text-kb-text-faint' => $step['status'] === 'pending',
                                    ])>
                                        @if ($step['status'] === 'done')
                                    ...
                                    <div class="mt-2 px-1 w-full">
                                        <div class="text-xs font-semibold text-kb-text-primary leading-tight line-clamp-2">{{ $step['title'] }}</div>
        ...
        @if ($recentDiagnoses->count() > 0)
            <div>
                <div class="flex items-center justify-between mb-4">
                    <flux:heading level="2" class="font-display text-lg font-semibold text-kb-text-primary">Riwayat Diagnosis</flux:heading>
                    <a href="{{ route('diagnosis.history') }}" class="text-sm font-medium text-kb-blue-electric hover:underline">
                        Lihat Semua
                    </a>
                </div>
                ...
                <div class="space-y-3">
                    @foreach ($recentDiagnoses as $diagnosis)
                        <a href="{{ route('diagnosis.show', $diagnosis) }}" wire:navigate class="block">
                            <flux:card class="p-4 rounded-2xl border-kb-border bg-kb-surface-1 card-glow cursor-pointer">
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                            <span class="text-xs font-semibold text-kb-text-muted uppercase tracking-wide">
    ...
    @else
        ...
        <flux:card class="p-12 text-center rounded-2xl border-kb-border bg-kb-surface-1">
            <div class="w-16 h-16 rounded-2xl bg-kb-blue-electric flex items-center justify-center mx-auto mb-4 shadow-brand">
